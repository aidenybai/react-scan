...
export async function gotoFixture(page: Page, fixtureUrl = FIXTURE_URL): Promise<void> {
  ...
  await page.waitForFunction(
    () => typeof window.__REACT_SCAN__?.ReactScanInternals !== "undefined",
    { timeout: 15_000 },
  );
  // Install a render counter by patching the onRender option on the signal
  await page.evaluate(() => {
    window.__E2E_RENDER_COUNT__ = 0;
    ...
    if (internals?.options) {
      ...
      internals.options.value = {
        ...prev,
        onRender: (...arguments_: Array<unknown>) => {
          window.__E2E_RENDER_COUNT__ = (window.__E2E_RENDER_COUNT__ ?? 0) + 1;
          prevOnRender?.(...arguments_);
        },
      };
    }
  });
...
export async function getRenderCount(page: Page): Promise<number> {
  return page.evaluate(() => window.__E2E_RENDER_COUNT__ ?? 0);
}
  ...
  return page.evaluate(
    ({ start, t }) => {
      return new Promise<number>((resolve) => {
        const check = () => {
          const current = window.__E2E_RENDER_COUNT__ ?? 0;
          if (current > start) {
...
export async function getInspectStateKind(page: Page): Promise<InspectStateKind | null> {
  return page.evaluate(() => {
    const scan = window.__REACT_SCAN__;
...
export function overlayPanel(page: Page): Locator {
  return page.locator(TOOLBAR_SELECTORS.panel);
}
