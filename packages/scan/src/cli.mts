...
const program = new Command().name("react-scan").description("React Scan CLI").version(VERSION);
  ...
  .action(async (opts) => {
    ...
    try {
      ...
      if (project.framework === "unknown") {
        ...
        console.log(
          pc.dim("  Visit https://github.com/aidenybai/react-scan#install for manual setup.\n"),
        );
        process.exit(1);
      }
