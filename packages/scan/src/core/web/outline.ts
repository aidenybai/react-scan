...
interface SerializedPendingOutline {
	rect: {
		height: number;
		width: number;
		x: number;
...
interface SerializedOutlineLabel {
	alpha: number;
	outline: SerializedPendingOutline;
	text: string | null;
	color: { r: number; g: number; b: number };
}

interface DrawGroupedOutlinesContext {
	canvas: OffscreenCanvas;
	pendingLabeledOutlines: Array<SerializedOutlineLabel>;

	dpi: number;
}

const drawPendingOutlines = new SmolWorker(() => {
	// WARNING
	// This entire function is completely isolated from the entire module
	// you cannot reference any JS identifiers from outside this function.
	const MONO_FONT =
		"Menlo,Consolas,Monaco,Liberation Mono,Lucida Console,monospace";
	return async ({
		canvas,
		pendingLabeledOutlines,
		dpi,
	}: DrawGroupedOutlinesContext) => {
		const ctx = canvas.getContext("2d");

		if (!ctx) {
			throw new Error("Missing OffscreenCanvasRenderingContext2D");
		}
...
function serializeOutlineLabel(
	array: Array<OutlineLabel>,
): Array<SerializedOutlineLabel> {
	const newArr: Array<SerializedOutlineLabel> = [];
	for (let i = 0, len = array.length; i < len; i++) {
		...
		newArr[i] = {
			...
			outline: {
				rect: {
					x: item.outline.rect.x,
					y: item.outline.rect.y,
					width: item.outline.rect.width,
					height: item.outline.rect.height,
				},
			},
		};
	}
...
export const fadeOutOutline = (
	...
	const serialized = serializeOutlineLabel(pendingLabeledOutlines);
	drawPendingOutlines
		.call(
			{
				canvas: canvas as OffscreenCanvas,
				pendingLabeledOutlines: serialized,
			},
			{
				transfer: [canvas],
				sync: !(canvas instanceof OffscreenCanvas),
			},
		)
		.catch((error: unknown) => {
			console.error(error);
