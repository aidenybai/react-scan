...
// AHFDSKJAHFKLASDJF WHAT ARE YOU DOING PAVEL
...
export interface SectionData {
  ...
  // if i had stats her maybe its trivial lets see
  // test: never;
}
...
export const collectInspectorData = (fiber: Fiber): InspectorDataResult => {
  ...
  if (fiber.memoizedProps) {
    ...
    // so i guess needs to be for each fiber that's it
    // there's no way this is stateful, right?
    for (const change of changes) {
